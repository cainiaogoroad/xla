...
class CostZeroGenrator : public CostGenerator {
 public:
  CostZeroGenrator(int mean, int std, int seed)
      : CostGenerator(mean, std, seed), mean_(mean), std_(std) {}
  int operator()() {
    int value = CostGenerator::operator()();
    if (value < mean_ - std_) {  // 1 delta
      return 0;
    }
    return value;
  }
...
class AutoReorderingTest : public HloTestBase {
 protected:
  void SetUp() override {
    setenv("XLA_AUTOREORDER_TIMEOUT", "60", 1);
    setenv("XLA_AUTOREORDER_WORKER", "32", 1);
  }
  void TearDown() override {
    unsetenv("XLA_AUTOREORDER_TIMEOUT");
    unsetenv("XLA_AUTOREORDER_WORKER");
  }
  bool RunLongtimeTest() {
    const char* env = getenv("XLA_AUTOREORDER_LONGTIME_TEST");
    if (env == nullptr) {
      return false;
    }
    if (std::strcmp(env, "true") == 0 || std::strcmp(env, "1") == 0) {
      return true;
    }
    return false;
  }
...
ENTRY %elementwise {
 ...
 public:
  ...
  std::string GetInstructionsOrderString(HloModule* hlo_module) {
    ...
    // FIX: after finetune,there will be start-start pair
  ...
  StatusOr<HloComputation*> MakeTestComputation(HloModule* module) {
    ...
    // d01 dot with p0
    auto d01_dot_p0 = builder.AddInstruction(
        HloInstruction::CreateDot(shape, ar_done0, p0, dot_dnums,
                                  precision_config),
        "d01_dot_p0");
    // d23 dot with p1
    auto d23_dot_p1 = builder.AddInstruction(
        HloInstruction::CreateDot(shape, ar_done1, p1, dot_dnums,
                                  precision_config),
        "d23_dot_p1");
    // d01_dot_p0 add p2
    ...
    auto reducescater_ret0 = builder.AddInstruction(
        HloInstruction::CreateAsyncDone(reduce_shape, async_start0));
    ...
    auto reducescater_ret1 = builder.AddInstruction(
        HloInstruction::CreateAsyncDone(reduce_shape, async_start1));
  ...
  StatusOr<HloComputation*> MakeRandomComputation(
      HloModule* module, SavedInstLatencyEstimator* estimator,
      const uint32_t inst_nums = 100, 
      uint8_t max_deps = 5,
      const double communication_rate = 0.1f,
      std::mt19937 gen = std::mt19937{kRandomSeed},
      CostGenerator cost_gen = CostGenerator(50, 5, kRandomSeed),
      bool enable_random_replica_group = false) {
    ...
    */
    ...
    HloComputation::Builder builder(
        absl::StrCat("N", inst_nums, "R", communication_rate));
    ...
    for (size_t i = 0; i < inst_nums; i++) {
      ...
      if (i % insert_comm_every == 0) {
        ...
        for (auto& dep : comm_deps) {
          ...
          if(enable_random_replica_group && random_gen() > 60){
            
            replica_groups = CreateReplicaGroups({{0, 9}});
          }
          else{
            replica_groups = CreateReplicaGroups({{0, 1}});
          }
...
TEST_F(AutoReorderingTest, TestContainer) {
  std::deque<int> test_queue;
  std::unordered_set<int> toRemoveSet = {2, 4, 6, 8};
  std::deque<int> return_inst_queue;
  for (int i = 0; i < 10; i++) {
    test_queue.push_back(i);
  }
  test_queue.erase(std::remove_if(test_queue.begin(), test_queue.end(),
                                  [&](int ele) {
                                    bool removed = false;
                                    if (toRemoveSet.count(ele) > 0) {
                                      return_inst_queue.push_back(ele);
                                      removed = true;
                                    }
                                    return removed;
                                  }),
                   test_queue.end());
  EXPECT_EQ(return_inst_queue.size(), 4);
  EXPECT_EQ(test_queue.size(), 6);
  std::cout << "test_queue:" << test_queue.size() << std::endl;
  for (int i = 0; i < test_queue.size(); i++) {
    std::cout << test_queue[i] << std::endl;
  }
  std::cout << "return_inst_queue:" << return_inst_queue.size() << std::endl;
  for (int i = 0; i < return_inst_queue.size(); i++) {
    std::cout << return_inst_queue[i] << std::endl;
  }
}
...
ENTRY e {
 ...
 %ar-start = bf16[8192,4096] all-reduce-start(bf16[8192,4096] %ag-done),to_apply=%add, channel_id=2, replica_groups={{0,1,2,3,4,5,6,7}}, use_global_device_ids=true
 %ar-done = bf16[8192,4096] all-reduce-done(%ar-start )
  %add-ret = bf16[8192,4096] add(%ag-done,%ar-done)
  ...
  // numel_bytes should be 1024*4096*2=8388608
  EXPECT_EQ(numel_bytes, 8388608);
  ...
  EXPECT_EQ(numel_bytes, 1024 * 4096 * 4 * 8);
...
ENTRY SyncTensorsGraph.23_spmd {
  ...
  auto gpu_latency_estimator = std::make_unique<xla::GpuLatencyEstimator>();
  std::unique_ptr<LatencyEstimator> latency_estimator;
  int pointer_size_ = 4;
...
TEST_F(AutoReorderingTest, ReorderPassAOpt) {
  setenv("XLA_AUTOREORDER_METHOD", "aopt", 1);
  absl::Cleanup env_cleanup = [&]() { unsetenv("XLA_AUTOREORDER_METHOD"); };
  ...
  int pointer_size_ = 4;
  Backend& test_backend = backend();
  auto gpu_device_info = TestGpuDeviceInfo::RTXA6000DeviceInfo(se::CudaComputeCapability::Ampere());
  const int64_t scheduler_mem_limit = xla::gpu::GetSchedulerMemoryLimit(
      hlo_module.get(), gpu_device_info, pointer_size_);
  SchedulerConfig config = GetSchedulerConfig(scheduler_mem_limit);
  auto gpu_latency_estimator = std::make_unique<xla::GpuLatencyEstimator>();
  ...
  // place)
  auto gpu_latency_estimator2 = std::make_unique<xla::GpuLatencyEstimator>();
...
TEST_F(AutoReorderingTest, GetSpaceCost) {
  std::srand(kRandomSeed);
  // testcase: 1. param 2. normal inst 3. tuple inst
  auto hlo_module = CreateNewUnverifiedModule();
  auto gpu_latency_estimator = std::make_unique<SavedInstLatencyEstimator>();
  SchedulerConfig sched_config = GetDefaultSchedConfig();
  auto st = MakeTestComputation(hlo_module.get());
  ...
  //f32[4,256,256]=4*256*256*4
  const HloInstruction* add_4 =
      computation->GetInstructionWithName("add.4");
  EXPECT_EQ(AutoReorderPass::GetSpaceCost(add_4), 1048576);
  // tuple (f32[4,256,256]{2,1,0}, f32[4,256,256]{2,1,0}, f32[2,256,256]{2,1,0})
  const HloInstruction* tuple =
      computation->GetInstructionWithName("tuple");
  EXPECT_EQ(AutoReorderPass::GetSpaceCost(tuple), 5242880);
}
TEST_F(AutoReorderingTest, FinetuneOrder) {
  std::srand(kRandomSeed);
  auto hlo_module = CreateNewUnverifiedModule();
  auto gpu_latency_estimator = std::make_unique<SavedInstLatencyEstimator>();
  SchedulerConfig sched_config = GetDefaultSchedConfig();
  HloComputation::Builder builder(absl::StrCat(TestName()));
  Shape shape = ShapeUtil::MakeShape(F32, {4, 256, 256});
  ...
  auto p3 =
      builder.AddInstruction(HloInstruction::CreateParameter(3, shape, "p3"));
  auto p4 =
      builder.AddInstruction(HloInstruction::CreateParameter(4, shape, "p4"));
  // allreduce-p0
  // r0 = custom-call: p1,p2
  // allreduce-p3,p4
  // m0 = dot(p0,p3)
  // m1 = dot(r0,p4)
  // some other case?

  auto add_reducer = MakeReduction(HloOpcode::kAdd, hlo_module.get());
  auto precision_config = DefaultPrecisionConfig(2);

  HloInstruction* ar0 = builder.AddInstruction(
      HloInstruction::CreateAllReduceStart(
          shape, {p0}, add_reducer,
          /*replica_groups=*/CreateReplicaGroups({{0, 1}}),
          /*constrain_layout=*/false, /*channel_id=*/1,
      ...
      "allreduce_start0");
  HloInstruction* ar_done0 = builder.AddInstruction(
      HloInstruction::CreateUnary(shape, HloOpcode::kAllReduceDone, ar0),
      "ar_done0");
  ...
  dot_dnums.add_rhs_batch_dimensions(0);
  auto d01_dot_p0 = builder.AddInstruction(
      HloInstruction::CreateDot(shape, p1, p1, dot_dnums, precision_config),
      "p1_dot_p2");

  HloInstruction* ar_p3 = builder.AddInstruction(
      HloInstruction::CreateAllReduceStart(
          shape, {p3}, add_reducer,
          /*replica_groups=*/CreateReplicaGroups({{0, 1}}),
          /*constrain_layout=*/false, /*channel_id=*/1,
          /*use_global_device_ids=*/true),
      "allreduce_start_p3");
  HloInstruction* ar_done_p3 = builder.AddInstruction(
      HloInstruction::CreateUnary(shape, HloOpcode::kAllReduceDone, ar_p3),
      "ar_done_p3");
  HloInstruction* ar_p4 = builder.AddInstruction(
      HloInstruction::CreateAllReduceStart(
          shape, {p4}, add_reducer,
          /*replica_groups=*/CreateReplicaGroups({{0, 1}}),
          /*constrain_layout=*/false, /*channel_id=*/1,
          /*use_global_device_ids=*/true),
      "allreduce_start_p4");
  HloInstruction* ar_done_p4 = builder.AddInstruction(
      HloInstruction::CreateUnary(shape, HloOpcode::kAllReduceDone, ar_p4),
      "ar_done_p4");
  auto p1_dot_p2 = builder.AddInstruction(
      HloInstruction::CreateDot(shape, p1, p1, dot_dnums, precision_config),
      "p1_dot_p2");
  auto p3_dot_p4 = builder.AddInstruction(
      HloInstruction::CreateDot(shape, ar_done_p3, ar_done_p4, dot_dnums,
                                precision_config),
      "p3_dot_p4");
  std::vector<HloInstruction*> last_n_insts = {p3_dot_p4, d01_dot_p0};
  auto last_ret = builder.AddInstruction(
      HloInstruction::CreateTuple(absl::MakeSpan(last_n_insts)));
  auto computation = builder.Build();
  VLOG(2) << "create computation success,test name" << TestName();
  ...
  // Node cost must after AddEntryComputation,so that instruction have
  // unique_id
  auto computation_ptr =
      hlo_module->AddEntryComputation(std::move(computation));

  gpu_latency_estimator->SetInstructionCost(d01_dot_p0, 5);
  gpu_latency_estimator->SetInstructionCost(p1_dot_p2, 5);
  gpu_latency_estimator->SetInstructionCost(p3_dot_p4, 5);

  gpu_latency_estimator->SetInstructionCost(ar0, 1);
  gpu_latency_estimator->SetInstructionCost(ar_done0, 1);
  gpu_latency_estimator->SetInstructionBetween(ar_done0, 3);
  gpu_latency_estimator->SetInstructionBetween(ar_done_p3, 3);
  gpu_latency_estimator->SetInstructionBetween(ar_done_p4, 3);

  // let module have one schedule
  StatusOr<HloSchedule> schedule_or_status = ScheduleModule(hlo_module.get(), [](const BufferValue& buffer) {
        return ShapeUtil::ByteSizeOf(buffer.shape(),
                                     /*pointer_size=*/sizeof(void*));
      });
  EXPECT_TRUE(schedule_or_status.ok());
  HloSchedule schedule = schedule_or_status.value();
  absl::Status status = hlo_module->set_schedule(std::move(schedule));
  // TF_RETURN_IF_ERROR(hlo_module->set_schedule(std::move(schedule)));
  EXPECT_TRUE(status.ok());
  status = schedule.Verify();
  EXPECT_TRUE(status.ok());

  auto status2 = RunScheduler(hlo_module.get(), sched_config,
                              std::move(gpu_latency_estimator));
  EXPECT_TRUE(status2.ok());
  auto insts = hlo_module->schedule()
                   .sequence(hlo_module->entry_computation())
                   .instructions();
  std::vector<absl::string_view> inst_name_list;
  inst_name_list.reserve(insts.size());
  for (auto inst : insts) {
    if (inst->opcode() == HloOpcode::kParameter) {
      continue;
    }
    inst_name_list.push_back(inst->name());
    // std::cout << inst->ToShortString() << std::endl;
  }
  EXPECT_EQ(absl::StrJoin(inst_name_list, ","),
            absl::StrJoin(
                std::vector<absl::string_view>(
                    {"allreduce_start_p3", "allreduce_start_p4", "p1_dot_p2.1",
                     "allreduce_start0", "ar_done_p3", "ar_done_p4", "ar_done0",
                     "p1_dot_p2", "p3_dot_p4", "tuple"}),
                ",")
...
TEST_F(AutoReorderingTest, ReorderPassWithRandomZero) {
  // GTEST_SKIP() << "Skipping single test";
  std::srand(kRandomSeed);
  auto hlo_module = CreateNewUnverifiedModule();
  auto gpu_latency_estimator = std::make_unique<SavedInstLatencyEstimator>();
  ...
  auto st = MakeRandomComputation(hlo_module.get(), gpu_latency_estimator.get(),
                                  ...
                                  CostZeroGenrator(50, 5, kRandomSeed));
  // std::cout<<hlo_module->ToString()<<std::endl;
  EXPECT_TRUE(st.ok());
  ...
  std::cout << "RunLatencyHidingScheduler:" << xla_lhs_cost << std::endl;
...
TEST_F(AutoReorderingTest, ReorderPassWithRandom) {
  ...
  EXPECT_TRUE(CheckParameterInst(hlo_module.get()));
  auto auto_reorder_cost = statics.value().total_cycles;
  std::cout << "ReorderPassWithRandom:" << auto_reorder_cost <<"memory_pressure_peak:"<<statics.value().memory_pressure_peak << std::endl;
  ...
  auto post_order_cost = statics.value().total_cycles;
  EXPECT_TRUE(CheckParameterInst(hlo_module.get()));
  std::cout << "MakeInstructionPostOrder:" << post_order_cost <<"memory_pressure_peak:"<<statics.value().memory_pressure_peak << std::endl;
  ...
  EXPECT_TRUE(statics.ok());
  auto xla_lhs_cost = statics.value().total_cycles;
  std::cout << "RunLatencyHidingScheduler:" << xla_lhs_cost <<"memory_pressure_peak:"<<statics.value().memory_pressure_peak << std::endl;
  auto insts = hlo_module->schedule()
                     .sequence(hlo_module->entry_computation())
                     .instructions();
  std::string xla_lhs_order_file = "/tmp/xla_lhs_order.txt";
  std::ofstream xla_lhs_order_file_out(xla_lhs_order_file);
  for (auto inst : insts) {
    xla_lhs_order_file_out << inst->ToString() << std::endl;
  }
  EXPECT_LE(auto_reorder_cost, post_order_cost);
  EXPECT_LE(auto_reorder_cost, xla_lhs_cost);
}
TEST_F(AutoReorderingTest, ReorderPassWithRandomInnerCommunication) {
  // GTEST_SKIP() << "Skipping single test";
  std::srand(kRandomSeed);
  auto hlo_module = CreateNewUnverifiedModule();
  auto gpu_latency_estimator = std::make_unique<SavedInstLatencyEstimator>();
  ...
  auto st = MakeRandomComputation(hlo_module.get(), gpu_latency_estimator.get(),
                                  ...
                                  std::mt19937{kRandomSeed},
                                  CostZeroGenrator(50, 5, kRandomSeed),
                                  /*enable_random_replica_group*/ true);
  // std::cout<<hlo_module->ToString()<<std::endl;
  EXPECT_TRUE(st.ok());
  VLOG(2) << "finish random computation make" << std::endl;
  auto gpu_latency_estimator2 = gpu_latency_estimator->clone();
...
TEST_F(AutoReorderingTest, ReorderPassDataAnalyse) {
  ...
  auto cost_gen = CostZeroGenrator(50, 5, kRandomSeed);
  int repeat_time = 1;
  uint32_t nnodes = 100;
  ...
  csv_out
      << "exp_id,nnodes,communication_rate,ortools_cost,aopt_cost,post_order_cost,"
         "xla_hiding_order_cost,ortools_memory_pressure_peak,aopt_memory_pressure_peak,post_order_memory_pressure_peak,xla_hiding_memory_pressure_peak"
      << std::endl;
  for (auto communication_rate : communication_rates) {
    for (size_t i = 0; i < repeat_time; i++) {
      ...
      EXPECT_TRUE(statics.ok());
      auto auto_reorder_cost = statics.value().total_cycles;
      auto auto_reorder_memory_pressure_peak = statics.value().memory_pressure_peak;
      // compare post order vs reorder
      auto post_insts_order =
          hlo_module->entry_computation()->MakeInstructionPostOrder();
      hlo_module->schedule().set_sequence(hlo_module->entry_computation(),
      ...
      EXPECT_TRUE(statics.ok());
      auto post_order_cost = statics.value().total_cycles;
      auto post_order_memory_pressure_peak = statics.value().memory_pressure_peak;
      // run LatencyHidingScheduler for compare
      // NOTICE:  DO NOT using gpu_latency_estimator after
      // std::move(gpu_latency_estimator)
      ...
      auto xla_hiding_memory_pressure_peak = statics.value().memory_pressure_peak;
      csv_out << i << "," << nnodes << "," << communication_rate << ","
              << auto_reorder_cost << "," << post_order_cost << ","
              << xla_hiding_order_cost << "," << auto_reorder_memory_pressure_peak << ","
              << post_order_memory_pressure_peak << "," << xla_hiding_memory_pressure_peak << std::endl;
    }
  }
}
TEST_F(AutoReorderingTest, ReorderPassAOptDataAnalyse) {
  if (!RunLongtimeTest()) {
    GTEST_SKIP() << "Skipping single test because longtime";
  }
  ...
  // communication rate from 0.05 to 0.95,step is 0.05
  std::ofstream csv_out("/tmp/test_ret_aopt.csv");
  csv_out
      << "exp_id,nnodes,communication_rate,ortools_cost,aopt_cost,post_order_cost,"
         "xla_hiding_order_cost,ortools_memory_pressure_peak,aopt_memory_pressure_peak,post_order_memory_pressure_peak,xla_hiding_memory_pressure_peak"
      << std::endl;
  for (auto communication_rate : communication_rates) {
    for (size_t i = 0; i < repeat_time; i++) {
      ...
      auto gpu_latency_estimator2 = gpu_latency_estimator->clone();
      auto gpu_latency_estimator3 = gpu_latency_estimator->clone();
      //hard to make same computation
      auto gpu_latency_estimator4 = gpu_latency_estimator->clone();
      // run AutoReorder for compare
      // get running time cost
      // call lambda,set env
      
      auto statics = [&](){
        setenv("XLA_AUTOREORDER_METHOD", "aoptremote", 1);
        absl::Cleanup env_cleanup = [&]() { unsetenv("XLA_AUTOREORDER_METHOD"); };
      ...
      EXPECT_TRUE(statics.ok());
      auto auto_reorder_aopt_cost = statics.value().total_cycles;
      auto auto_reorder_aopt_memory_pressure_peak = statics.value().memory_pressure_peak;
      //run ortools
      statics = [&](){
        auto start = std::chrono::steady_clock::now();
        auto status = RunScheduler(hlo_module.get(), sched_config,
                                  std::move(gpu_latency_estimator4));
        EXPECT_TRUE(status.ok());
        auto statics = GetModuleCost(hlo_module.get(), sched_config,
                                   gpu_latency_estimator2.get());
        return statics;
      ...
      EXPECT_TRUE(statics.ok());
      auto auto_reorder_ortools_cost = statics.value().total_cycles;
      auto auto_reorder_ortools_memory_pressure_peak = statics.value().memory_pressure_peak;
      ...
      auto xla_hiding_memory_pressure_peak = statics.value().memory_pressure_peak;
      csv_out << i << "," << nnodes << "," << communication_rate << ","
              << auto_reorder_ortools_cost << ","
              << auto_reorder_aopt_cost << "," 
              << post_order_cost << ","
              << xla_hiding_order_cost << "," 
              << auto_reorder_ortools_memory_pressure_peak << ","
              << auto_reorder_aopt_memory_pressure_peak << "," 
              << post_order_memory_pressure_peak << "," 
              << xla_hiding_memory_pressure_peak << std::endl;
    }
  }
}
TEST_F(AutoReorderingTest, DAGSolver) {
  auto solver_ = absl::make_unique<LinearProgramScheduler<
      LPContainerDAG<const HloInstruction*>, const HloInstruction*>>();
